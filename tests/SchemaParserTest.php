...
class SchemaParserTest extends BaseTest{
    function testGetModifiedSchema(){
        ...
        if($actual !== $expected){
            $assertDiff($expected, $actual, "The previously displayed modified schema paths were removed!  Please review the difference between the expected & actual modified schema summary files.");

            $assertDiff($actual, $expected, "The previously displayed modified schema paths were added!  If this was intentional, you may want to update the expected modified scheme summary with the actual summary.");
        }
