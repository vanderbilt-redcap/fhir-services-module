...
class FHIRServicesExternalModule extends \ExternalModules\AbstractExternalModule
{
	...
	public function getOntologySystems() {
	...
	public function redcap_every_page_top() {
		if ($this->isPage('DataEntry/record_home.php')) {
			$this->hookRecordHome();
		} elseif ($this->isPage('Design/online_designer.php')) {
	...
	public function onDataDictionaryUploadPage() {
		?>
	...
	private function setFormName($project_id, $form_name, $new_form_name) {
		...
		if ($status < 1) {
			...
			while (strpos($new_form_name, "__") !== false) {
				$new_form_name = str_replace("__", "_", $new_form_name);
			}
			while (substr($new_form_name, 0, 1) == "_") {
				$new_form_name = substr($new_form_name, 1);
			}
			while (substr($new_form_name, -1) == "_") {
				$new_form_name = substr($new_form_name, 0, -1);
			}
			while (is_numeric(substr($new_form_name, 0, 1))) {
				$new_form_name = substr($new_form_name, 1);
			}
			while (substr($new_form_name, 0, 1) == "_") {
				$new_form_name = substr($new_form_name, 1);
		...
		// Logging
		if ($q1) {
			\Logging::logEvent("", $metadata_table, "MANAGE", $form_name, "form_name = '".db_escape($form_name)."'", "Rename data collection instrument");
		}
	...
	public function getQuestionnaireEDoc($projectId = null) {
	...
	public function toArray($fhirObjectOrArray) {
	...
	public function getFHIRUrlParts() {
	...
	public function getProjectAndRecordIdsFromFHIRUrl() {
	...
	public function getResourceUrlPrefix() {
		if (isset($_GET['pid'])) {
	...
	public function getRelativeResourceUrl($resource) {
	...
	public function getRecordFHIRId($projectId, $recordId) {
		return "$projectId-$recordId";
	}

	public function getInstanceFHIRId($pid, $record, $event, $form, $instance) {
		$form = str_replace('_', '-', $form);
		return $this->getRecordFHIRId($pid, $record) . "-$event-$form-$instance";
	}

	public function addIdentifier($resource, $projectId, $recordId) {
		if ($resource->getId()) {
	...
	public function formatREDCapDateTime($mixed) {
		return $this->getDateTime($mixed)->format('Y-m-d H:i');
	}

	public function formatREDCapDateTimeWithSeconds($mixed) {
		return $this->getDateTime($mixed)->format('Y-m-d H:i:s');
	}

	public function formatREDCapTime($mixed) {
	...
	public function isFHIRResource($o) {
		return $o instanceof FHIRResource;
	}

	public function buildBundle($compositionsPid, $compositionId) {
	...
	public function getOrganizationFromRecord($organization) {
	...
	public function saveResource($expectedType) {
	...
	public function questionnaireToDataDictionary($questionnaire) {
	...
	public function getREDCapChoices($item) {
	...
	public function parseREDCapChoices($redcapField) {
	...
	public function getCodeSystemUrl($fieldName) {
	...
	public function getFHIRAnswerOptions($redcapField) {
	...
	public function questionnaireResponseToREDCapExport($jsonOrXml) {
	...
	public function getRepeatingForms($eventId = null) {
		if ($eventId === null) {
			$eventId = $this->getEventId();
		}
	...
	public function walkQuestionnaire($parents, $fieldAction) {
		if (!is_array($parents)) {
	...
	public function isRepeating($item) {
		...
		if ($item->_getFHIRTypeName() === 'Questionnaire.Item') {
			...
			if ($type) {
				if (in_array($type, ['string', 'integer', 'decimal', 'dateTime'])) {
					...
					$type = 'notes';
				} elseif (in_array($type, ['choice', 'open-choice'])) {
					return 'dropdown';
				} elseif ($type === 'boolean') {
					return 'yesno';
	...
	public function getFHIRType($redcapField) {
		$type = $redcapField['element_type'];
		...
		if ($type === 'text') {
			if (empty($validation)) {
				return 'string';
			} elseif (strpos($validation, 'date_') === 0) {
				return 'date';
			} elseif (strpos($validation, 'datetime_') === 0) {
				return 'dateTime';
			} elseif ($validation === 'int') {
				return 'integer';
			} elseif ($validation === 'float') {
				return 'decimal';
			} elseif ($validation === 'time') {
				return 'time';
			}
		...
		} elseif (in_array($type, ['select', 'radio', 'yesno', 'truefalse'])) {
			return 'choice';
		} elseif ($type === 'checkbox') {
			return 'boolean';
		} elseif ($type === 'file' && empty($validation)) {
			return 'attachment';
		} elseif ($type === 'slider') {
			// not currently supported
	...
	public function getInstrumentName($group, $item = null, $displayName = false) {
	...
	public function getQuestionnaireUrl($section) {
		return $this->getUrl("questionnaire-options.php?section=$section");
	}

	public function getExtensionForMIMEType($contentType) {
	...
	public function getReceivedResources($whereClause = '') {
	...
	public function getReceivedResource($logId) {
		$logId = db_escape($logId);
		$result = $this->getReceivedResources("and log_id = $logId");
		return $result->fetch_assoc();
	}

	public function getFHIRResourceForRecord($projectId, $recordId) {
	...
	public function buildQuestionnaireResponse($questionnaire, $projectId, $recordId, $instances, $eventName = null) {
		$data = [];
	...
	public function createQuestionnaireAnswer($item, $value) {
		$type = $this->getValue($item->getType());
	...
	public function getRemoteFHIRServerUrl() {
	...
	public function respondAndExit($o) {
		header('Content-type: application/fhir+json');
		echo $this->jsonSerialize($o);
		exit();
	}

	public function createQuestionnaireItem($redcapField) {
	...
	public function handleActionTags($redcapField, &$item) {
	...
	public function parseActionTags($redcapField) {
	...
	private function getInitialValue($redcapField, $item, $value) {
		...
		} elseif ($fhirType === 'time') {
			$value = $this->formatFHIRTime($value);
		} elseif (in_array($fhirType, ['text', 'textarea'])) {
			$methodNameSuffix = 'String';
		} elseif ($fhirType === 'choice') {
	...
	public function getREDCapVersionDirURL() {
		return APP_PATH_WEBROOT_FULL . ltrim(APP_PATH_WEBROOT, '/');
	}

	public function createQuestionnaire($pid, $formName, $formDisplayName, $fields, $repeatingForms, $eventName = null) {
	...
	public function getDescriptiveVideoHTML($text, $url) {
	...
	public function getMappedFieldsAsBundle($projectId, $recordId) {
	...
	public function createBundle($resources, $type = 'collection') {
	...
	public function isRepeatableResource($type) {
		return $type !== 'Patient';
	}

	public function formatConsentCategories($values) {
		$categories = [];
	...
	public function getEConsentFHIRBundleForInstance($pid, $record, $form, $event, $instance) {
	...
	public function getEConsentFHIRBundle($args) {
		$type = $args['type'];
	...
	public function validateInBrowserAndDisplay($resource) {
		?>
	...
	public function getEConsentFormSettings($formName) {
	...
	public function getEConsentData($form, $record, $instance) {
		$Proj = new \Project();
	...
	public function sendToRemoteFHIRServer($resource) {
	...
	public function getResourceId($resourceName, $pid, $recordId, $formOrFieldName, $data) {
		if (!empty($recordId)) {
	...
	public function setAssociativeArrayValues(&$array, $keyToInsertAfter, $newValues) {
	...
	public function initResource(&$resource, $recordId, $formOrFieldName, $data) {
		$type = $resource['resourceType'];
		$copy = $resource;
	...
	public function getFormsForEventId($eventId) {
